...
using ping_atom = atom_constant<atom("stop")>;
using pong_atom = atom_constant<atom("ack")>;
...
behavior ping(event_based_actor* self, size_t num_pings) {
  ...
  return {
    [=](kickoff_atom, const actor& pong) {
      aout(self) << "client: send {" << to_string(ping_atom::value) << ", " <<  42 << "}" << endl;
      self->send(pong, ping_atom::value, int32_t(42));
      
      self->become (
        [=](pong_atom, int32_t value) -> void {
	      self->quit();
	      return;
