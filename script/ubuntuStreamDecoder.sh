...
echo "This script will start our docker container. Please give the path to a HDF5 File and the tag for the docker image you want to start."
echo -en '\n'
if [ ! -f $1 ] || [ $# -lt 2 ]; then 
  echo -en '\n'
  echo "Wrong Usage. Usage: ubuntuVideo.sh [ _FULL_PATH_TO_HDF5_FILE_ ] [ _CONTAINER_TAG_ ]"
  echo "Whitespaces in the path are not allowed!"
  echo "Example: script/ubuntuVideo.sh /home/file.hdf5 v1"  
  exit
fi  
